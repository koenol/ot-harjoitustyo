...
class TestMaksukortti(unittest.TestCase):
    ...
    def test_negatiivisen_summan_lataaminen_ei_nosta_kortin_saldoa(self):
        self.kortti.lataa_rahaa(-1000)

        self.assertEqual(str(self.kortti), "Kortilla on rahaa 10.00 euroa")

    def test_kortilla_pystyy_ostamaan_edullisen_lounaan_kun_kortilla_saldoa_edullisen_verran(self):
        kortti = Maksukortti(250)
        kortti.syo_edullisesti()

        self.assertEqual(kortti.saldo_euroina(), 0.0)

    def test_kortilla_pystyy_ostamaan_maukkaan_lounaan_kun_kortilla_saldoa_maukkaan_verran(self):
